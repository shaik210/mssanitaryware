...
import { sanitarySeries, sanitaryData } from "@/app/data/sanitarywareData";
...
export default function SeriesPage() {
  ...
  const products = sanitaryData[seriesId] || [];

  const [searchTerm, setSearchTerm] = useState<string>("");

  // ✅ Find the series image from faucetSeries data
  const series = sanitarySeries.find((item) => item.id === seriesId);
      ...
      >
        {filteredProducts.length > 0 ? (
          filteredProducts.map((item) => (
            <Link
              key={item.id}
              href={`/products/sanitaryware/${seriesId}/${item.id}`}
              style={{
                textDecoration: "none",
                color: "black",
